...
public class CompleteUserInfoContoller {
    ...
    public ResponseEntity<CompletePatientInfoResponse> completePatientInfo(@RequestHeader("Authorization") String jwt,
                                                                        @PathVariable Long userId,
                                                                        @RequestBody CompletePatientInformationsRequest request) throws DoctorException {



         String role = jwtProvider.getUserRoleFromToken(jwt);
         Long patientId = jwtProvider.getUserIdFromToken(jwt);
//        System.out.println("88888888888888888889888875445345354");
//        System.out.println(role);
//        System.out.println("88888888888888888889888875445345354");

        if(role.equals("PATIENT") && patientId.equals(userId) ){
            CompletePatientInfoResponse response = completeUserInformationService.completePatientInformation(request,userId);
            return ResponseEntity.status(HttpStatus.OK).body(response);
        }else{
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
